...
def calc_용신(사주_data):
    ...
    오행_십성 = {
        '비겁': 일간_오행,
        '식상': {'목': '화', '화': '토', '토': '금', '금': '수', '수': '목'}[일간_오행],
        '재성': {'목': '토', '화': '금', '토': '수', '금': '목', '수': '화'}[일간_오행],
        '관성': {'목': '금', '화': '수', '토': '목', '금': '화', '수': '토'}[일간_오행],
        '인성': {'목': '수', '화': '목', '토': '화', '금': '토', '수': '금'}[일간_오행],
    }
    
    # 가장 부족한 오행 찾기
    필요_오행들 = [오행_십성[x] for x in 용신_후보]
    부족한_오행 = min(필요_오행들, key=lambda x: 오행_count[x])
    ...
    return {
        ...
        '신강점수': f"{신강_점수}/8",
        '용신': 용신,
        '용신_설명': f"부족한 {용신}을(를) 보충",
        '희신': 희신,
        '희신_설명': f"{용신}을 생해주는 {희신}",
        '기신': 기신,
        '기신_설명': f"{용신}을 극하는 {기신}",
        '주의': '※ 참고용 자동분석입니다. 정확한 용신은 전문가 상담 필요',
    }
